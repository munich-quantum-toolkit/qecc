...
def generate_circuits(n: int, num_circuits: int, output_dir: Path) -> None:
    ...
    """
    depth = 2 * n
    folder = output_dir / str(n)
    folder.mkdir(parents=True, exist_ok=True)

    print(f"Generating {num_circuits} circuits for n={n}, depth={depth}...")

    for seed in range(num_circuits):
        qc = random_universal_circuit(num_qubits=n, depth=depth, seed=seed)
