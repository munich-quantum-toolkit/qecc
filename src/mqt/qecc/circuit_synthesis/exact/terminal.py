...
def add_clifford_unitary_terminal(
    solver: z3.Solver,
    n: int,
    tableau_x_final: list[list[z3.BoolRef]],
    tableau_z_final: list[list[z3.BoolRef]],
    ...
    Args:
        ...
        n: Number of qubits.
        tableau_x_final: Final X part of tableau (2n x n).
        tableau_z_final: Final Z part of tableau (2n x n).
    ...
    if not allow_permutation:
        ...
        selector = [[z3.Bool(f"unitary_selector_{i}_{q}") for q in range(n)] for i in range(n)]
...
def add_clifford_isometry_terminal(
    ...
    Args:
        ...
        tableau_x_final: Final X part of tableau.
        tableau_z_final: Final Z part of tableau.
    ...
    pivot = [z3.Bool(f"pivot_{q}") for q in range(n)]
    ...
    if allow_permutation:
        ...
        selector = [[z3.Bool(f"logical_selector_{i}_{q}") for q in range(n)] for i in range(k)]
...
def add_stabilizer_state_terminal(
    ...
    Args:
        solver: Z3 solver instance.
        n: Number of qubits.
        tableau_x_final: Final X part of tableau (n x n).
        tableau_z_final: Final Z part of tableau (n x n).
    """
...
def add_css_isometry_terminal(
    ...
    k: int,
    m_x: int,
    matrix_final: list[list[z3.BoolRef]],
) -> None:
    ...
    Args:
        ...
        matrix_final: Final CSS matrix [L; H].
    """
    if k == 0:
    ...
    pivot = [z3.Bool(f"css_pivot_{q}") for q in range(n)]
...
def add_css_state_terminal(
    ...
    Args:
        ...
        matrix_final: Final CSS matrix (m_x x n).
    """
    # Introduce pivot variables
    pivot = [z3.Bool(f"css_state_pivot_{q}") for q in range(n)]
